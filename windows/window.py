...
def loginWindow():
    ...
    NewWindow = Button(frame2, text='Entrar', font=("Arial", 10), width=10, height=2, command=lambda: configFunctions.functions.validate_login(et_login.get(), et_senha.get()))
...
def clientWindow():
    ...
    clients = ttk.Treeview(frame_clients, columns=(
        'ID', 'Nome', 'Contato', 'CPF'), show='headings', height=20)
    ...
    search_buttom = Button(frame_clients, text='Pesquisar', font=("Arial", 10), command=lambda: configFunctions.functions.search(clients, search_entry))
    search_buttom.pack(side=TOP, expand=False)

    client_delete_buttom = Button(frame_clients, background='sea green', font=("Arial", 10), text="Deletar Cliente", width=15, height=3,
                                  border=5, command=lambda: configFunctions.functions.deletar(clients, clientsInformations))
    client_delete_buttom.pack(side=RIGHT, fill=X, expand=False, padx=35, pady=35)

    client_SUM_buttom = Button(frame_clients, background='sea green', font=("Arial", 10), text="Somar Dividas", width=15, height=3, border=5,
    ...
    statistcs_buttom = Button(frame_clients, background='sea green', text='Estatísticas de Vendas', width=15, border=5, height=3, command=lambda: configFunctions.functions.year_datas(yearchoosen.get()))
    statistcs_buttom.pack(side=BOTTOM, fill=X, expand=False, padx=35, pady=35)

    anos = []
    configFunctions.functions.listYears(anos)
    text = StringVar()
    text.set("SELECIONE O ANO")
    yearchoosen = ttk.Combobox(frame_clients, width=27, state="readonly", textvariable=text)
    yearchoosen['values'] = anos
    yearchoosen.pack(side=BOTTOM, expand=False)
    ...
    client_register_buttom = Button(frame_clients_register, background='sea green', text="Cadastrar Cliente", font=("Arial", 10), width=15, height=3, border=5,
                                    command=lambda: configFunctions.functions.NewUser
                                    (name_entry.get(), contact_entry.get(), cpf_entry.get(), clients))
    client_register_buttom.pack(side=TOP, fill=Y, expand=False, pady=30)
    ...
    clientsInformations = ttk.Treeview(frame_clients_informations, columns=(
        'id', 'Valor da Compra', 'Data'), show='headings', height=10)
    ...
    paymentButtom = Button(frame_clients_informations, background='sea green', text="Pagar", font=("Arial", 10), padx=15, pady=5, command=lambda: configFunctions.functions.payment(clients, clientsInformations, paymentEntry.get()))
    ...
    newPurchaseButtom = Button(frame_clients_informations, background='sea green', text="Comprar", font=("Arial", 10), padx=5, pady=5, command=lambda: configFunctions.functions.newPurchase(clients, newPurchaseEntry.get(), clientsInformations))
    newPurchaseButtom.pack(side=TOP, expand=False)

    total_label = Label(frame_clients_informations, text='Total: R$', font=("Arial", 25))
...
def statistcsWindow(valorVendas, meses, valorQuitado):
    # Gráficos de barras 3D
    fig = plt.figure(figsize=(8, 6))
    ax = plt.axes(projection="3d")
    z_pos = [0] * len(valorVendas)
    x_size = np.ones(len(valorVendas))
    y_size = np.ones(len(valorVendas))
    ax.bar3d(valorVendas, meses, z_pos, x_size, y_size, valorQuitado, color='blue')
